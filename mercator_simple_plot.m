...
function mercator_simple_plot(mercator_dir,date,variable,region,depth,arrows)
...
nc = ncgeodataset(mercator_dir); % Assign a ncgeodataset handle.
...
if ~any(strcmp(standard_vars,variable))  
    ...
    if any(strcmp(slab_vars,variable))
        mercator_simple_plot_layer(nc,date,variable,region)
        return
        
    elseif strcmp(variable,'velocity') 
        mercator_simple_plot_velocity(nc,date,region,depth,arrows) 
        return 
