...
Functions are named using a two-part system. The prefix (argo_, bathymetry_, general_, etc.) indicates the appropriate data source, while the suffix (\_build, \_profiles, \_section, etc.) indicates the action performed. The **\_build** suffix is special because it loads raw data into uniform structs (e.g. argo, cruise, hycom, mercator, woa, wod). Uniform structs are then compatable with general_ functions.
